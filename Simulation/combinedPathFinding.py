...
def alterPath(path, prevSpot, start, grid):
	#start is the current path start spot
	sRow = start.row
	sCol = start.col #get the row and col position of the start on the grid
	firstSpot = path[0]
	tempOrientation = firstSpot[2]
	orientation = prevSpot[2]
	tempX = prevSpot[0][0]
	tempY = prevSpot[0][1] #get x y coords of prevSpot
	insertCorrection = [] #correction required to get the robot to face the right direction
	if orientation == 0 and tempOrientation == 90: #rev twice and add a left
		insertCorrection.append([(tempX - 30, tempY),'Rev', 180])
		insertCorrection.append([(tempX - 60, tempY),'Rev', 180])
		insertCorrection.append([(tempX - 60, tempY),'L', 0])
		insertCorrection.append([(tempX, tempY),'Rev', 270])
	elif orientation == 0 and tempOrientation == 270:
		insertCorrection.append([(tempX - 30, tempY),'Rev', 180])
		insertCorrection.append([(tempX - 60, tempY),'Rev', 180])
		insertCorrection.append([(tempX - 60, tempY),'R', 0])
		insertCorrection.append([(tempX, tempY),'Rev', 90])
	elif orientation == 90 and tempOrientation == 180:
		insertCorrection.append([(tempX, tempY + 30),'Rev', 270])
		insertCorrection.append([(tempX, tempY + 60),'Rev', 270])
		insertCorrection.append([(tempX, tempY + 60),'L', 90])
		insertCorrection.append([(tempX, tempY),'Rev', 0])
	elif orientation == 90 and tempOrientation == 0:
		insertCorrection.append([(tempX, tempY + 30),'Rev', 270])
		insertCorrection.append([(tempX, tempY + 60),'Rev', 270])
		insertCorrection.append([(tempX, tempY + 60),'R', 90])
		insertCorrection.append([(tempX, tempY),'Rev', 180])
	elif orientation == 180 and tempOrientation == 90:
		insertCorrection.append([(tempX + 30, tempY),'Rev', 0])
		insertCorrection.append([(tempX + 60, tempY),'Rev', 0])
		insertCorrection.append([(tempX + 60, tempY),'R', 180])
		insertCorrection.append([(tempX, tempY),'Rev', 270])
	elif orientation == 180 and tempOrientation == 270:
		insertCorrection.append([(tempX + 30, tempY),'Rev', 0])
		insertCorrection.append([(tempX + 60, tempY),'Rev', 0])
		insertCorrection.append([(tempX + 60, tempY),'L', 180])
		insertCorrection.append([(tempX, tempY),'Rev', 90])
	elif orientation == 270 and tempOrientation == 0:
		insertCorrection.append([(tempX, tempY - 30),'Rev', 90])
		insertCorrection.append([(tempX, tempY - 60),'Rev', 90])
		insertCorrection.append([(tempX, tempY - 60),'L', 270])
		insertCorrection.append([(tempX, tempY),'Rev', 180])
	elif orientation == 270 and tempOrientation == 180:
		insertCorrection.append([(tempX, tempY - 30),'Rev', 90])
		insertCorrection.append([(tempX, tempY - 60),'Rev', 90])
		insertCorrection.append([(tempX, tempY - 60),'R', 270])
		insertCorrection.append([(tempX, tempY),'Rev',0])
	elif orientation == 0 and tempOrientation == 180:
		lClear = True #clear on the left
		rClear = True #clear on the right
		bClear = True #clear on the back

		#find the row/col of the last known safe spot
		sRow -= 2 #grid[sRow-2][sCol] is not a barrier
		for i in range(1,3):
			if grid[sRow][sCol-i].is_barrier(): 
				rClear = False
				break
		for i in range(1,3):
			if grid[sRow-i][sCol].is_barrier(): 
				bClear = False
				break
		for i in range(1,3):
			if grid[sRow][sCol+i].is_barrier(): 
		...
		if rClear and bClear:
			insertCorrection.append([(tempX, tempY), 'R', 180])
			insertCorrection.append([(tempX-60, tempY-60), 'R', 270])
			insertCorrection.append([(tempX, tempY), 'Rev', 0])
		elif lClear and bClear:
			insertCorrection.append([(tempX, tempY), 'L', 180])
			insertCorrection.append([(tempX-60, tempY+60), 'L', 90])
			insertCorrection.append([(tempX, tempY), 'Rev', 0])
		else:
			insertCorrection.append([(tempX, tempY), 'R', 180])
			insertCorrection.append([(tempX-60, tempY+60), 'Rev', 270])
			insertCorrection.append([(tempX-60, tempY+60), 'R', 90])

	elif orientation == 90 and tempOrientation == 270:
		lClear = True #clear on the left
		rClear = True #clear on the right
		bClear = True #clear on the back
		...
		if rClear and bClear:
			insertCorrection.append([(tempX, tempY), 'R', 270])
			insertCorrection.append([(tempX-60, tempY+60), 'R', 0])
			insertCorrection.append([(tempX, tempY), 'Rev', 90])
		elif lClear and bClear:
			insertCorrection.append([(tempX, tempY), 'L', 270])
			insertCorrection.append([(tempX+60, tempY+60), 'L', 180])
			insertCorrection.append([(tempX, tempY), 'Rev', 90])
		else:
			insertCorrection.append([(tempX, tempY), 'R', 270])
			insertCorrection.append([(tempX+60, tempY+60), 'Rev', 0])
			insertCorrection.append([(tempX+60, tempY+60), 'R', 180])

	elif orientation == 180 and tempOrientation == 0:
		lClear = True #clear on the left
		rClear = True #clear on the right
		bClear = True #clear on the back
		...
		if rClear and bClear:
			insertCorrection.append([(tempX, tempY), 'R', 0])
			insertCorrection.append([(tempX+60, tempY+60), 'R', 90])
			insertCorrection.append([(tempX, tempY), 'Rev', 180])
		elif lClear and bClear:
			insertCorrection.append([(tempX, tempY), 'L', 0])
			insertCorrection.append([(tempX+60, tempY-60), 'L', 270])
			insertCorrection.append([(tempX, tempY), 'Rev', 180])
		else:
			insertCorrection.append([(tempX, tempY), 'R', 0])
			insertCorrection.append([(tempX+60, tempY-60), 'Rev', 90])
			insertCorrection.append([(tempX+60, tempY-60), 'R', 270])

	elif orientation == 270 and tempOrientation == 90:
		lClear = True #clear on the left
		rClear = True #clear on the right
		bClear = True #clear on the back
		...
		if rClear and bClear:
			insertCorrection.append([(tempX, tempY), 'R', 90])
			insertCorrection.append([(tempX+60, tempY-60), 'R', 180])
			insertCorrection.append([(tempX, tempY), 'Rev', 270])
		elif lClear and bClear:
			insertCorrection.append([(tempX, tempY), 'L', 90])
			insertCorrection.append([(tempX-60, tempY-60), 'L', 0])
			insertCorrection.append([(tempX, tempY), 'Rev', 270])
		else:
			insertCorrection.append([(tempX, tempY), 'R', 270])
			insertCorrection.append([(tempX-60, tempY-60), 'Rev', 180])
			insertCorrection.append([(tempX-60, tempY-60), 'R', 0])
	insertCorrection.extend(path)
...
def reconstruct_path(came_from, current, draw, start):
	...
	directForSimu[len(directForSimu)-1][2] = directForSimu[len(directForSimu)-2][2]
	#copy the orientation of the previous spot
	for i in directForSimu:
		print(i)
	print("------------------------------")
	return directForSimu #return found path to one goal
	...
	while i < len(directForSimu):
		if directForSimu[i][1] == 'L' or directForSimu[i][1] == 'R':
			if previousTurn == -1:
				...
				#add revs
				if orient == 0:
					directForSimu.insert(i, [(tempX , tempY),'Rev', 180])
				elif orient == 90:
					directForSimu.insert(i, [(tempX, tempY), 'Rev', 270])
				elif orient == 180:
					directForSimu.insert(i, [(tempX, tempY), 'Rev', 0])
				elif orient == 270:
					directForSimu.insert(i, [(tempX, tempY), 'Rev', 90])
	...
	for i in range(len(path)-1):
		...
		if tempOrientation != orientation:
			...
			if orientation == 0 and tempOrientation == 90:
				if path[i-1][1] == 'T': #extremely tight turn
					path[i-1][1] = 'L'
					path[i-1][0] = (tempX-30, tempY)
